...
export const chunkText = (text, chunkSize = 500, overlap = 50) => {
  ...
  for (const paragraph of paragraphs) {
    ...
    let paragraphWordCount = paragraphWords.length;
  ...
  // Add the final remaining chunk after the loop
  if (currentChunk.length > 0) {
    chunks.push({
      content: currentChunk.join("\n\n"),
      chunkIndex: chunkIndex,
...
export const findRelevantChunks = (chunks, query, maxChunks = 3) => {
  ...
  const scoredChunks = chunks.map((chunk, index) => {
    ...
    for (const word of queryWords) {
      ...
      const extraPartials = Math.max(0, partialMatches - exactMatches);
      if (extraPartials > 0) {
        score += extraPartials * 1.5;
        if (exactMatches === 0) wordsFound++;
      }
    }
  ...
  // Fallback: If no matches, return the first few chunks
  if (results.length === 0 && chunks.length > 0) {
    console.log("[Search] No matches found, falling back to first chunks.");
    return chunks.slice(0, maxChunks).map((chunk) => ({
      content: chunk.content,
      chunkIndex: chunk.chunkIndex,
      pageNumber: chunk.pageNumber,
